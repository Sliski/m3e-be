...
router.get('/my-games', (req, res) => {
  ...
  console.log(`${email} collects games history.`);
  db().collection('games').find({
    $or: [
      {
        'players.creator.user.email': req.session.user.email,
        'players.creator.deleted': false,
      },
      {
        'players.opponent.user.email': req.session.user.email,
        'players.opponent.deleted': false,
      },
    ],
  }, {
    projection: {
  ...
  }).toArray((err, games) => res.send({
    status: 'OK',
    games: games.map((game) => {
      const userRole = game.players.creator.user.email === email ? 'creator' : 'opponent';
      return {
        _id: game._id,
        finished: game.players[userRole].step === dbController.ENCOUNTER_STEPS.FINISHED_GAME,
        leader: game.players[userRole].crew.leader,
...
router.get('/delete-game/:gameId', (req, res) => {
  const { gameId } = req.params;
  if (!req.session.Auth || !ObjectID.isValid(gameId)) {
    return res.redirect('/rejected');
  }
  console.log(`${req.session.user.email} deletes game ${gameId}.`);
  db().collection('games').findOne(new ObjectID(gameId)).then((game) => {
    if (!game) {
      return res.send({
        status: 'rejected',
    ...
    db().collection('games').updateOne(game, {
      $set: {
        [`players.${currentPlayer}.step`]: dbController.ENCOUNTER_STEPS.FINISHED_GAME,
        [`players.${currentPlayer}.deleted`]: true,
      },
    }).then(() => {
      req.app.get('io').to(gameId).emit('newAppState', currentPlayer);
      res.redirect('/');
...
router.get('/game-report/:gameId', (req, res) => {
  const { gameId } = req.params;
  if (!req.session.Auth || !ObjectID.isValid(gameId)) {
    return res.redirect('/rejected');
  }
  console.log(`${req.session.user.email} get report of game ${gameId}.`);
  db().collection('games').findOne(new ObjectID(gameId)).then((game) => {
    if (!game) {
      return res.send({
        status: 'rejected',
    ...
    const currentPlayer = dbController.currentPlayer(req, game);
    const secondPlayer = dbController.secondPlayer(currentPlayer);
    if (!currentPlayer) {
      return res.send({
        status: 'rejected',
        info: 'Incorrect user.',
    ...
    if (game.players[currentPlayer].deleted) {
      return res.send({
        status: 'rejected',
    ...
    return res.send({
      ...
      report: {
        date: game.created,
        deploymentId: game.appState.deploymentId,
        strategyId: game.appState.strategyId,
        schemesIds: game.appState.schemesIds,
        gameNotes: game.players[currentPlayer].notes,
        currentPlayer: {
          schemes: game.players[currentPlayer].schemes,
          strategyScore: game.players[currentPlayer].strategyScore,
          crew: game.players[currentPlayer].crew,
        },
        secondPlayer: game.players[secondPlayer] ? {
          schemes: game.players[secondPlayer].schemes,
          strategyScore: game.players[secondPlayer].strategyScore,
          crew: game.players[secondPlayer].crew,
        } : null,
      },
    });
  });
...
router.post('/report-crash', (req, res) => {
  console.log(`${req.session.user || req.session.user.email} app crashes.`);

  db().collection('crashes').insertOne({
    body: req.body,
    created: new Date(),
  }, (error) => {
    if (error) return res.redirect('/rejected');
