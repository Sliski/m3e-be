...
router.get('/create-game/:encounterId/:multiplayer', (req, res) => {
  if (!req.session.Auth) return res.redirect('/rejected');
  console.log(`${req.session.user.email} creates game.`);

  const decodedEncounter = dbController.decodeEncounterId(req.params.encounterId);
  if (!decodedEncounter) {
  ...
  const appState = {
    ...decodedEncounter,
    round: 1,
    multiplayer: req.params.multiplayer === 'true',
  };
